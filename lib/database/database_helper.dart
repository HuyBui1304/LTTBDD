...
class DatabaseHelper {
  ...
  /// Teacher -> Subjects (via creatorId = UID) -> Students (via subjectIds)
  Future<List<Student>> getStudentsByTeacher(String teacherUid) async {
    try {
      // 1. Get all subjects of the teacher
      ...
      // 2. Find students whose subjectIds contain any of these subject IDs
      ...
      for (final student in allStudentsList) {
        if (student.subjectIds != null && student.subjectIds!.isNotEmpty) {
          // Check if student studies any of the teacher's subjects
