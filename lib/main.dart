...
void main() async {
  ...
  // Skip if users already exist to avoid re-creation and rate limiting
  try {
    final hasUsers = await SampleDataService.instance.checkSampleUsersExist();
    if (!hasUsers) {
      debugPrint('No sample users found, creating all sample data...');
      await SampleDataService.instance.initializeAllSampleData();
    } else {
      debugPrint('Sample users exist, only creating missing subjects if any...');
      // initializeAllSampleData will skip users that already exist
      await SampleDataService.instance.initializeAllSampleData();
    }
  } catch (e) {
