...
class _AddSubjectScreenState extends State<AddSubjectScreen> {
  ...
  // Session dates (dynamic - can be added/removed)
  List<DateTime?> _sessionDates = [];
  List<TimeOfDay?> _sessionStartTimes = [];
  List<TimeOfDay?> _sessionEndTimes = [];
  List<String> _sessionTimeSlots = []; // per-session slot: 'morning', 'afternoon', 'custom'
  ...
  void _addSession() {
    setState(() {
      ...
      String timeSlot = 'morning'; // default to morning slot
      
      startTime = const TimeOfDay(hour: 7, minute: 30);
      endTime = const TimeOfDay(hour: 11, minute: 30);
      
      if (_autoFillSessions && _sessionDates.isNotEmpty) {
        // Auto: place next session 7 days after the last one, reuse its time slot
        ...
        // Inherit start/end times from the last session
        ...
        // Manual: add session with today's date
        _sessionDates.add(DateTime(
          now.year,
          now.month,
          now.day,
  ...
  void _toggleAutoFillSessions(bool value) {
    setState(() {
      ...
      if (value && _sessionDates.isNotEmpty) {
        // Auto-fill: space sessions 7 days apart from the first, preserve each session's time slot
  ...
  void _onSessionTimeSlotChanged(int index, String? value) {
    ...
    setState(() {
      ...
      if (value == 'morning') {
        ...
        // Custom - keep selected times or fall back to defaults
